...
function Dialog({setEmpleados, setDialog, setEdit, edit, setError}) {
    ...
    let res=null
    if(edit[0]){
        res = await PromiseFetchPUT(`https://api-vimo-production.up.railway.app/empleados`,{cedula, nombres, apellidos ,correo, telefono, direccion, fechaNacimiento})
    } else{
                
     res= await PromiseFetchGET(`https://api-vimo-production.up.railway.app/empleados/${cedula}`)
    if(res.one!=null){
        setError('Esta cedula ya existe en la base de datos')
        return
    }
    res = await PromiseFetchPOST(`https://api-vimo-production.up.railway.app/empleados`,{cedula, nombres, apellidos ,correo, telefono, direccion, fechaNacimiento})

    }
    res = await PromiseFetchGET(`https://api-vimo-production.up.railway.app/empleados`)
    setDialog(false)
    setEdit(false)
    setEmpleados(res.allData)
}
...
{        data.map((empleado)=>(
        <tr key={empleado.id}>
            <td className="max-w-[100px] text-center break-words">{empleado.cedula}</td>
            <td className="max-w-[150px] text-center break-words">{empleado.nombres}</td>
            <td className="max-w-[250px] text-center break-words">{empleado.apellidos}</td>
            <td className="max-w-[200px] text-center break-words">{empleado.correo}</td>
            <td className="max-w-[90px] text-center break-words">{empleado.telefono}</td>
            <td className="max-w-[250px] text-center break-words">{empleado.direccion}</td>
            <td className="max-w-[250px] text-center break-words">{parseData(empleado.fechaNacimiento)}</td>
...
export default function Empleados({dialog, setDialog}) {
    const[empleados, setEmpleados]=useState([])  
    const {data, loading} = useFetch('https://api-vimo-production.up.railway.app/empleados')
...
<div className="table-container h-96 flex flex-col">
    ...
    {dialog && <Dialog setEmpleados={setEmpleados} setDialog={setDialog} edit={edit} setEdit={setEdit} setError={setError}></Dialog>}
    {error!=""  && <Fail setError={setError} msg={error}></Fail>}
    {!loading ? (
    <RenderData data={empleados} setEdit={setEdit} setDialog={setDialog}></RenderData>
