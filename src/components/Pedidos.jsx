...
function AddProdu({setDetalle, setShowProdu, setDialog, setError, showProdu}) {
...
function Dialog({setVentas, detalle, edit, setEdit, setDetalle, setDialog, setShowProdu,setRenderP, setError}) {
   ...
   function handleClose(){
    ...
    async function handleSubmit(event){
        ...
        if(edit[0]){
            await PromiseFetchPUT(`https://api-vimo-production.up.railway.app/pedidoVenta/ventas`, {id,fecha,pais,detalleCompra:detalle,valorTotal:total})
            setDetalle([])
            setDialog(false)
            return
        }
        ...
        if(res.one!=null){
         ...
         await PromiseFetchPOST(`https://api-vimo-production.up.railway.app/pedidoVenta`,{id, fecha, pais, detalleVenta:detalle, valorTotal:total, concepto:"venta"})
        ...
        <button className='ml-auto mr-[10px] block'><FontAwesomeIcon className='fa-2xl' icon={faCircleXmark} onClick={handleClose} /></button>
                    ...
                    <div className="inline-block hover:border-formInputs1 w-[280px] border-2 rounded-2xl py-1 px-2">
                        ...
                        <div className="inline-block hover:border-formInputs1 w-[280px] border-2 rounded-2xl py-1 px-2">
                            ...
                            <input required defaultValue={edit[0]? edit[1].id:null} name='id' type='number' min="1" className="block w-[95%] text-1xl font-sans font-semibold px-3 w-3/4 h-10 py-2 outline-0" placeholder="Camisa blabla"></input>
                        </div>
...
{data.map((venta)=>(
        <tr key={venta.id}>
            ...
            <td className="max-w-[250px] text-center break-words">{venta.montoAdeudado}</td>
...
export default function Ventas({dialog, setDialog}) {
    ...
    const {data, loading} = useFetch('https://api-vimo-production.up.railway.app/pedidoVenta/pedidos')
...
<div className="table-container h-96 flex flex-col">
   ...
   {dialog &&  <Dialog setError={setError} detalle={detalle} edit={edit} setEdit={setEdit} setDetalle={setDetalle} setVentas={setVentas} setDialog={setDialog} setShowProdu={setShowProdu} setRenderP={setRenderP}></Dialog>}
    ...
    {showProdu && <AddProdu setShowProdu={setShowProdu} setDialog={setDialog} setDetalle={setDetalle} setError={setError} showProdu={showProdu}></AddProdu>}
